...
edgelist = pd.read_csv('C:/Users/BOSS/Desktop/project/3/edgelist.csv')
nodelist = pd.read_csv('C:/Users/BOSS/Desktop/project/3/nodes.csv')
...
# Calculate a list of nodes with odd degree
...
# Compute the min weight matching.
# Note: max_weight_matching uses the 'weight' attribute by default as the attribute to maximize.
odd_matching_dupes = nx.algorithms.max_weight_matching(g_odd_complete, True)

# Convert the matching to a list of deduped tuples
...
# Create the augmented graph: add the min weight matching edges to g
...
def create_eulerian_circuit(graph_augmented, graph_original, starting_node=None):
    ...
    for edge in naive_circuit:
        ...
        if edge_data[0]['trail'] != 'augmented':
            # If `edge` exists in the original graph, grab the edge attributes and add to the Eulerian circuit.
...
print('Number of edges in the circuit: {}'.format(len(euler_circuit)))
...
def create_cpp_edgelist(euler_circuit):
    ...
    Combine duplicate edges and keep track of their sequence and the number of walks.
    Parameters:
...
bbox = {'ec': [1, 1, 1, 0], 'fc': [1, 1, 1, 0]}  # hack to label edges over the line (rather than breaking up the line)
